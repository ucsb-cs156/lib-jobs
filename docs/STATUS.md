...
Which app repos are on which version of `lib-jobs`, as of 2026-08-24.
...
- **v0.3.2** (`JobContext.checkCancellation()` made public — same
  ...
  verified); citelines, frontiers, and dining remain on v0.2.0 (happycows
  on v0.1.6) — the version column will update as each app's bump PR
  merges.
...
done 2026-08-24). **Remaining: citelines, frontiers, dining** — order
among these three not yet decided.

**Standing action items for every remaining app's v0.3.x rollout PR**
(citelines, frontiers, dining — Phill, 2026-08-21), both found during
scaffold's live QA testing and unrelated to lib-jobs itself, but newly
consequential now that jobs run on a single-threaded executor by default:
...
v0.3.0→v0.3.2 bump; courses hit both proactively (fixed before any live
bug report) in #331.
