...
testing and unrelated to lib-jobs itself, but newly consequential now
that jobs run on a single-threaded executor by default:
...
v0.3.0→v0.3.2 bump; courses and frontiers each hit both proactively
(fixed before any live bug report) in #331 and #700 respectively —
frontiers by far the largest instance of item 2, with 6 job classes and
10 total checkpoints needed.
